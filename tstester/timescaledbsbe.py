...
class TimescaleDBSBE(StorageBackend):
    """A storage backend that uses a TimescaleDB database for storage of observations,
    per observation metadata, and per time series metadata."""

    def __init__(self, verbose, pg_conn_info):
        super().__init__(verbose, 'TimescaleDB')
    ...
    def set_obs(self, ts, times, values):
        ...
        if self._verbose:
            ...
            print('    ts: {}\n    times: (size: {}), values: (size: {})'.format(
                ts.__dict__, len(times), len(values)), file=sys.stderr)
