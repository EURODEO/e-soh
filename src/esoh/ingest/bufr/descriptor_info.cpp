...
std::list<Descriptor> descriptorTree(Descriptor d, bool recoursive, TableD *td,
                                     int l) {
...
int main(int argc, char *argv[]) {
  ...
  TableB *tb = new TableB(
      "/usr/share/eccodes/definitions/bufr/tables/0/wmo/37/element.table");
  // TableC * tc = new TableC("./bufrtables/BUFRCREX_CodeFlag_en.txt");
  TableD *td = new TableD(
      "/usr/share/eccodes/definitions/bufr/tables/0/wmo/37/sequence.def");

  if (argc <= 1) {
    std::cout << "Print Descriptor information\n";
    std::cout << "Usage: " << std::string(argv[0])
              << " [tableD_file] descriptor\n";
