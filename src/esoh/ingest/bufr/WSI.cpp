...
WSI::WSI(std::string s)
{
    from_string(s);
}

WSI::WSI(const char * c )
{
    from_string(std::string(c));
}
...
int WSI::getWigosIdSeries() const
{
    return wigos_id_series;
}

uint16_t WSI::getWigosIssuerId() const
{
    return wigos_issuer_id;
}

uint16_t WSI::getWigosIssueNum() const
{
    return wigos_issue_num;
}

std::string WSI::getWigosLocalId() const
{
    return wigos_local_id;
}
...
bool operator<(const WSI &lhs, const WSI &rhs)
{
    if( lhs.wigos_id_series < rhs.wigos_id_series ) return true;
    if( lhs.wigos_id_series > rhs.wigos_id_series ) return false;
    if( lhs.wigos_issuer_id < rhs.wigos_issuer_id ) return true;
    if( lhs.wigos_issuer_id > rhs.wigos_issuer_id ) return false;
    if( lhs.wigos_issue_num < rhs.wigos_issue_num ) return true;
    if( lhs.wigos_issue_num > rhs.wigos_issue_num ) return false;
    if( lhs.wigos_local_id  < rhs.wigos_local_id  ) return true;
