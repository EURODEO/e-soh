...
ssize_t SectionBase::length() const
{
    return len;
}
...
bool Section1::optSection() const
{
    return optional_section;
}

int Section1::getMasterTable() const
{
    return master_table;
}

int Section1::getCentre() const
{
    return centre;
}

int Section1::getSubCentre() const
{
    return subcentre;
}

int Section1::getUpdateSeqNum() const
{
    return upd_seq_num;
}

int Section1::getOptionalSelection() const
{
    return optional_section;
}

int Section1::getDataCategory() const
{
    return data_category;
}

int Section1::getDataSubCategory() const
{
    return int_data_subcategory;
}

int Section1::getLocalDataSubCategory() const
{
    return local_data_subcategory;
}

int Section1::getVersionMaster() const
{
    return version_master;
}

int Section1::getVersionLocal() const
{
...
bool Section1::fromBuffer(uint8_t * buf, int size, uint8_t edition)
{
    ...
    if( !SectionBase::fromBuffer(buf,size) ) return false;
    ...
    if( edition >= 4 ) local_data_subcategory = buffer[12];
    else eshift--;
    ...
    if( edition >= 4 ) bufr_time.tm_sec = buffer[21+eshift];
    else eshift--;
...
std::ostream & operator<<( std::ostream & os, Section1 & sec)
{
    ...
    os << "International Data Subcategory: " << static_cast<int>(sec.int_data_subcategory) << "\n";
    os << "Local Data Subcategory: " << static_cast<int>(sec.local_data_subcategory) << "\n";
    ...
    for(unsigned int i=0; i<sec.local_data.size(); ++i) os << sec.local_data[i] << " " ;
    os << "\n";
...
std::ostream & operator<<( std::ostream & os, Section2 & sec)
{
    ...
    for(unsigned int i=0; i<sec.local_data.size(); ++i)
    {
        os << std::hex << "0x" << std::setw(2) << std::setfill('0') << static_cast<unsigned int>(sec.local_data[i]) << " " ;
    }
    os << std::dec << "\n";
...
bool Section3::isObserved() const
{
    return (obs_comp & 0x80);
}

bool Section3::isCompressed() const
{
    return (obs_comp & 0x40);
}

uint16_t Section3::subsetNum() const
{
    return subsets;
}
...
uint64_t Section4::bitSize() const
{
    return bits.size();
}
