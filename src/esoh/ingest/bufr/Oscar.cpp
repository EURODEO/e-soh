...
const rapidjson::Value & Oscar::findStation(WSI wsi) const
{
    ...
    if( ret != oscar_not_found ) return ret;

    for( auto & v : stations )
    {
        if( v.second.HasMember("wigosStationIdentifiers") && v.second["wigosStationIdentifiers"].Size() > 0 )
        {
