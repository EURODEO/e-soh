...
class NorBufr : public Section1, public Section2, public Section3, public Section4
{
...
public:
    ...
    std::string getValue(const Descriptor & d,std::string s, bool with_unit = true) const;
    void setTableB( TableB * tb) { tabB = tb; }
    void setTableC( TableC * tc) { tabC = tc; }
    void setTableD( TableD * td) { tabD = td; }
    uint64_t length() const;
    void print(const DescriptorId df, const std::string filter, const DescriptorId dv) const;
...
private:
    ...
    uint64_t uncompressDescriptor(std::list<DescriptorId>::iterator & it, ssize_t & sb, ssize_t & subsetsb, uint16_t * repeatnum = 0);
