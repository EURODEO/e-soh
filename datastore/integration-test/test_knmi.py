...
def test_find_series_single_station_single_parameter(grpc_stub):
    request = dstore.GetObsRequest(
        filter=dict(
            ...
            instrument=dstore.Strings(values=["relative_humidity_2.0_mean_PT10M"]),
        )
    )
...
def test_find_series_all_stations_single_parameter(grpc_stub):
    request = dstore.GetObsRequest(filter=dict(instrument=dstore.Strings(values=["relative_humidity_2.0_mean_PT10M"])))
    response = grpc_stub.GetObservations(request)
...
input_params_polygon = [
    (
        ...
        ["relative_humidity_2.0_mean_PT10M"],
        ["06260", "06348", "06356"],
    ),
    (
    ...
    (
        ...
        ((52.15, 4.90), (52.15, 5.37), (51.66, 5.37), (51.66, 4.90)),
        ["wind_from_direction_2.0_mean_PT10M", "relative_humidity_2.0_mean_PT10M", "air_temperature_2.0_mean_PT10M"],
        ["06260", "06348", "06356"],
    ),
    (
        # One station within
