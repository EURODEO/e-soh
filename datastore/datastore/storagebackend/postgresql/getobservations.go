...
func scanObsRow(rows *sql.Rows) (*datastore.ObsMetadata, int64, error) {
	...
	rv := reflect.ValueOf(&obsMdata)
	for i, goName := range obsStringMdataGoNames {
		val, ok := colVals0[i].(string)
		if !ok {
			return nil, -1, fmt.Errorf(
				"colVals0[%d] not string: %v (type: %T)", i, colVals0[i], colVals0[i])
		}
...
func getObs(db *sql.DB, request *datastore.GetObsRequest, obs *[]*datastore.Metadata2) (retErr error) {
	...
	obsMap := make(map[int64][]*datastore.ObsMetadata) // observations per time series ID
